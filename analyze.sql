...
\gset
select round(count(*)::numeric/:局数, 3) as 和了率 from winnings inner join players on winnings.actor = players.id where players.name = :'NAME';
select round(count(*)::numeric/(select count(*) from winnings inner join players on winnings.actor = players.id where players.name = :'NAME'), 3) as 自摸率 from winnings inner join players on winnings.actor = players.id where players.name = :'NAME' and winnings.actor = winnings.target;
...
select round(avg(score), 0) as 平均最終持ち点 from results inner join players on player_id = players.id where players.name = :'NAME';
select seat as 起家, count(seat) as 回数 from players where name = :'NAME' group by seat order by seat;
