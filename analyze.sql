...
select round(avg(winnings.delta), 3) as 平均得点 from winnings inner join players on winnings.actor = players.id where players.name = :'NAME';
select round(avg(winnings.delta), 3) as 平均失点 from winnings inner join players on winnings.actor != players.id and winnings.target = players.id where players.name = :'NAME';
